...
import { getProfile, setOnboardingDone, updateProfile, type LanguageCode, type LanguageGoal, type LanguageLevel, type LearningLang } from "@/src/storage";
...
function chip(label: string, active: boolean, onPress: () => void) {
  return (
    ...
    >
      <Text style={{ color: active ? "#fff" : "#000", fontWeight: "800" }}>{label}</Text>
    </TouchableOpacity>
  );
}
...
export default function Onboarding() {
  ...
  const canNext = learn.length > 0 && learn.every(x => x.level !== null);
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#fff", padding: 20 }}>
      <ScrollView>
        ...
        <View style={{ flexDirection: "row", flexWrap: "wrap" }}>
          ...
          {chip(i18n.t("goalExchange"), goal === "Intercambio", () => setGoal("Intercambio"))}
        </View>
        ...
        <View style={{ flexDirection: "row", flexWrap: "wrap", marginBottom: 8 }}>
          {learn.map((x) => {
            ...
            return chip(`${meta?.flag ?? ""} ${(meta?.label ?? x.lang)} (${x.level ?? "—"})`, active, () => setSelectedLang(x.lang));
          })}
        </View>

        {/* Niveles para ese idioma */}
        <View style={{ flexDirection: "row", flexWrap: "wrap" }}>
          {LEVELS.map((lvl) => chip(lvl, selected?.level === lvl, () => setLevelForSelected(lvl)))}
        </View>
        ...
        <Text style={{ marginTop: 10, fontSize: 12, opacity: 0.6 }}>
          * Para continuar tenés que seleccionar al menos 1 idioma y asignarle nivel a cada uno.
        </Text>
      </ScrollView>
    </SafeAreaView>
  );
