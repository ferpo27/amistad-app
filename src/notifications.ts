...
export async function sendPushNotification(
  ...
  try {
    ...
    // Cast a any para evitar error de tipo unknown
    const data = (result as any).data;

    if (response.status !== 200) {
      console.warn('Failed to send push notification:', data);
    }
  } catch (error) {
 ...
 */
export async function getAllScheduledNotifications(): Promise<Notifications.ScheduledNotification[]> {
