...
const DEFAULT: MyProfile = {
  ...
  country: "Argentina",
  nativeLang: "es",
  learning: [{ lang: "en", level: "B1" }, { lang: "ja", level: "A1" }],
...
export async function getMyProfile(): Promise<MyProfile> {
  ...
  if (!raw) {
    await AsyncStorage.setItem(KEY, JSON.stringify(DEFAULT));
    return DEFAULT;
  }
  try {
...
export async function updateMyProfile(patch: Partial<MyProfile>): Promise<MyProfile> {
  ...
  const next: MyProfile = { ...cur, ...patch } as MyProfile;
...
export async function addProfilePhoto(uri: string): Promise<MyProfile> {
  const cur = await getMyProfile();
  const next = { ...cur, photos: [uri, ...cur.photos.filter((p) => p !== uri)].slice(0, 9) };
  await AsyncStorage.setItem(KEY, JSON.stringify(next));
  return next;
}
...
export async function addStory(imageUri: string): Promise<MyProfile> {
  const cur = await getMyProfile();
  const story: ProfileStory = { id: uid("story"), imageUri, createdAt: Date.now(), caption: "" };
...
export async function deleteStory(storyId: string): Promise<MyProfile> {
  const cur = await getMyProfile();
  const next = { ...cur, stories: cur.stories.filter((s) => s.id !== storyId) };
  await AsyncStorage.setItem(KEY, JSON.stringify(next));
  return next;
}